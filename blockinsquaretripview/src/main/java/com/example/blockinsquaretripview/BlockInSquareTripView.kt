...
class BlockInSquareTripView(ctx : Context) : View(ctx) {
    ...
    data class BISTNode(var i : Int, val state : State = State()) {

        private var next : BISTNode? = null
        private var prev : BISTNode? = null
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawBISTNode(i, state.scale, paint)
        }
        ...
        fun getNext(dir : Int, cb : () -> Unit) : BISTNode {
            var curr : BISTNode? = prev
            if (dir == 1) {
                curr = next
            }
            if (curr != null) {
    ...
    data class BlockInSquareTrip(var i : Int) {

        private var curr : BISTNode = BISTNode(0)
